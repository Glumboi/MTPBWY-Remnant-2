...
public class Plugin : StandardPluginImplementations
{
    ...
    private const string _engineIniLocation = @"%USERPROFILE%\AppData\Local\Remnant2\Saved\Config";
    ...
    {
        return Directory.Exists(Environment.ExpandEnvironmentVariables(@"%USERPROFILE%\Saved Games\Remnant2"));
    }

    public void OpenGameSaveLocation()
    {
        Process.Start("explorer.exe", Environment.ExpandEnvironmentVariables(@"%USERPROFILE%\Saved Games\Remnant2"));
    }
